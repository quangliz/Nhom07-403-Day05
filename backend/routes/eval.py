...
from eval.logger import write_log, update_outcome, write_correction, load_logs, load_corrections
from eval.metrics.latency import compute_p95_latency
from eval.metrics.conversion import compute_conversion_rate, compute_satisfaction_rate, compute_unsure_rate
from eval.metrics.accuracy import run_accuracy_eval, check_disclaimer_compliance, run_batch_accuracy
...
async def log_correction(req: CorrectionRequest):
    ...
    return {"ok": True, "correction_id": cid, "message": "Merchant da duoc notify."}
...
async def dashboard(days: int = Query(7, ge=1, le=30)):
    ...
    kill = []
    if conversion.get("red_flag"):
        kill.append("Conversion < 15%")
    if unsure.get("stop_immediately"):
        kill.append("STOP: Hallucination allergen > 10%")
    if latency.get("red_flag"):
        kill.append("Latency p95 > 15s")
    return {
        "period_days": days,
        ...
        "metrics": {
            ...
            "disclaimer_compliance": disclaimer,
            "allergen_hallucination": unsure,
        },
        "kill_criteria_triggered": kill,
        "status": "STOP" if unsure.get("stop_immediately") else ("WARNING" if kill else "OK"),
    }
